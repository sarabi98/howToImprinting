...
Get file as dataframe and refine ID name and parental info
``` python
...
Get global table with all info from files in given paths
```python
def getTable(*paths):
    data=pd.DataFrame()
    for path in paths:
...
Ploting histogram of pvalues [0,1]
```python
def plotHistP(data,name):
    ...
    plt.xlim([0, 1])
```
Ploting histogram of mat/pat ratio p2-p1 [-1,1]
```python
def plotHistR(data,name,lim=False):
    ...
    if lim:
        plt.ylim([0,100])
```
Ploting bar graphs of individual sample expression for one particular gene
```python
def plotExpression(gene,name):
    fig, axes = plt.subplots(2,2)
    plt.figure(figsize=(40,20), dpi= 100, facecolor='w', edgecolor='k')
    fig.suptitle('Levels of expression for: '+name)
    c1=gene[gene['Type']=='C1KS']
    c1=c1.pivot(index='ID', columns='Parental', values='Z')
    c1=c1.sort_values('ID')
    c1.plot(ax=axes[0,0],kind='bar')
    axes[0, 0].set_title('C1')
    #plt.title("Histogram Mat/Pat P2-P1 ratio "+name)
    
    c2=gene[gene['Type']=='C2SK']
    c2=c2.pivot(index='ID', columns='Parental', values='Z')
    c2=c2.sort_values('ID')
    c2.plot(ax=axes[0,1],kind='bar')
    axes[0, 1].set_title('C2')

    sin=gene[gene['Type']=='SIN']
    sin=sin.pivot(index='ID', columns='Parental', values='Z')
    sin=sin.sort_values('ID')
    sin.plot(ax=axes[1,0],kind='bar')
    axes[1, 0].set_title('Sin')

    kaz=gene[gene['Type']=='KAZ']
    kaz=kaz.pivot(index='ID', columns='Parental', values='Z')
    kaz=kaz.sort_values('ID')
    kaz.plot(ax=axes[1,1],kind='bar')
    axes[1, 1].set_title('Kaz')
```
## Using data
...
```
Info of specific genes
```python
#getting info on a specific gene
...
```
Best candidates based on graphs

![image](https://github.com/ClemLasne/SHARED_SINKAZ/assets/94226596/c4869627-c230-49a2-8ffb-106b6ac088d8)
![image](https://github.com/ClemLasne/SHARED_SINKAZ/assets/94226596/90dc7aae-2904-48e6-9c5d-9e6a5965a7fb)
![image](https://github.com/ClemLasne/SHARED_SINKAZ/assets/94226596/e7303042-071c-4def-a95e-ea32edbebbf4)
...
![image](https://github.com/ClemLasne/SHARED_SINKAZ/assets/94226596/27a4b2ed-32d6-4f4f-988d-6b4b2f085996)

Location

d1463350857414 Chromosome_1 89014188 597

Not that much power

![image](https://github.com/ClemLasne/SHARED_SINKAZ/assets/94226596/fe3bd17f-fafe-49ae-9868-7e661b7c6950)

Similar to parent

![image](https://github.com/ClemLasne/SHARED_SINKAZ/assets/94226596/d2981f49-313a-4a89-89c8-5e0a9cedab39)


![image](https://github.com/ClemLasne/SHARED_SINKAZ/assets/94226596/610d7248-8900-4e6c-92d2-dc8b4226a246)
